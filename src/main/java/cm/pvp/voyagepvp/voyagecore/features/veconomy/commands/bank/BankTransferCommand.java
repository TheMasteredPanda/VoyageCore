...
import cm.pvp.voyagepvp.voyagecore.features.veconomy.commands.argument.check.BankExistsCheck;
...
public class BankTransferCommand extends VoyageCommand
{
    ...
    private BankTransferCommand(VEconomy feature)
    {
        ...
        bankExists.setCheckFunction(new BankExistsCheck(feature));
    ...
    public void execute(CommandSender sender, VoyageCommand command, LinkedList<String> arguments)
    {
        ...
        SharedAccount from = feature.getHandler().getAccessibleBanks(p.getUniqueId()).stream().filter(id -> feature.getAccount(id).getName().equals(arguments.get(0))).map(id -> feature.getAccount(id)).findFirst().get();
        ...
        if (feature.getInstance().getMojangLookup().lookup(arguments.get(1)).isPresent()) {
            ...
            SharedAccount to = feature.getHandler().getAccessibleBanks(p.getUniqueId()).stream().filter(id -> feature.getAccount(id).getName().equals(arguments.get(1))).map(id -> feature.getAccount(id)).findFirst().get();
